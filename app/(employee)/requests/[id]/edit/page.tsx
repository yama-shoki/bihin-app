...
import { dispatchPageError, ForbiddenError } from "@/app/lib/errors";
...
export default async function EditRequestPage({ params }: Props) {
  ...
  try {
    ...
    if (!canEditPurchaseRequest(user, request.applicant.id)) {
      throw new ForbiddenError("この申請を編集する権限がありません");
    }
    if (request.status !== "pending") {
      throw new ForbiddenError("申請中の状態でのみ編集できます");
    }
